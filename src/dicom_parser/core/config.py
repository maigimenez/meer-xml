#  -*- coding: utf-8 -*-
import ConfigParser
from config_variables import (TEMPLATES_SECTION, TEMPLATES_ROOT_PATH,
                              TEMPLATE_PACKAGE, I18N_INPUT, I18N,
                              DEFAULT_INPUT, DEFAULT, SETTINGS_PATH,
                              USER_SETTINGS_PATH, OUTPUT_DIRECTORIES_SECTION,
                              STRINGS_OUTPUT_OPTION, STRINGS,
                              XML_STRINGS_SECTION, PROPERTIES_OPTION,
                              STRING_TEMPLATES, EXTENSIONS_SECTION,
                              PROPERTIES_EXTENSION, STRING_TEMPLATES_PATH,
                              DEFAULT_STRINGS_SECTION, I18N_SECTION,
                              LEVEL_STRINGS,
                              MULTIPLE_PROPERTIES, ONTOLOGIES_SETTINGS,
                              ONTOLOGIES_PATH, ONTOLOGY_FILENAMES,
                              SETTINGS_EXTENSION, XML_EXTENSION, LAYOUTS,
                              ACTIVITIES, JAVA_EXTENSION, LEVEL_TAG,
                              LAYOUT_SETTINGS, CHILDREN_TAG,
                              MODEL_OUTOUT_OPTION, MODEL_FILE,
                              STRING_TEMPLATES_SECTION, LAYOUT_TEMPLATES,
                              LAYOUT_TEMPLATES_PATH, LAYOUT_TEMPLATES_SECTION)
...
def get_class_name(schema, code, parent_schema, parent_code):
    ...
    if ((parent_schema is None) or (parent_code is None)):
...
# TODO: There are 3 ways to instantiate a filename:
# get_template_filename, instantiate_filename and get_template_model_file.
# Merge!
...
def read_config(path=SETTINGS_PATH):
    ...
    if(config.read(path) == []):
        exit("The settings file is empty or it's not where it's supposed "
             "to be.\n It should be at ./settings"
             " (I was looking for this file: {0})".format(path))
    return config


# TODO: Hay 3 funciones para encontrar la ruta en la configuración:
# get_properties_path, get_filepath y get_filepath_ontology. Merge!
def get_filepath(filetype, language_code=None):
    ...
    try:
        ...
        user_settings = config.get(USER_SETTINGS_PATH,
                                   OUTPUT_DIRECTORIES_SECTION)
        ...
        if(exists(user_settings)):
            ...
            if (filetype == STRINGS and language_code):
                output_directory = user_config.get(OUTPUT_DIRECTORIES_SECTION,
                                                   STRINGS_OUTPUT_OPTION)
                #Get the path for the filename for the language_code
...
def get_properties_path(language):
    ...
    return join(config.get(OUTPUT_DIRECTORIES_SECTION, PROPERTIES_OPTION),
                language + config.get(EXTENSIONS_SECTION,
...
def get_template_filename(template):
    ...
    if (template in STRING_TEMPLATES):
        template_path = config.get(TEMPLATES_SECTION, STRING_TEMPLATES_PATH)
    ...
    # Layout templates
        ...
        exit("The template path for: {0}, has not been found."
             .format(template))
    # Return the path to template file.
    return (join(root_path, template_path), template_name)


#TODO: If interpolation is false get_property_interpolation
# and get_property do exactly the same. Merge!
def get_property(section, option):
    """ Returns a configuration property given a section and an option
...
def get_language_section_options(languages="", path=SETTINGS_PATH):
    ...
    if (languages == DEFAULT_INPUT):
        ...
        exit("The code you have enter for internationalization is not "
             "supported.\n The options are \"default\"(for english) and "
             "\"i18n\"(for spanish as first language and english "
             "as second language)")
...
def get_ontology_level(ontology_id, tree_level, dicom_tag=None,
                       languages_tag=None, language=None):
    """
    ...
    dicom_tag -- dicom xml tag used to find out corresponding
    ...
    if (dicom_tag is not None):
        ...
        #If the language code does not exists, we will write comments
...
def get_substitution_dictionary(language_code, section, template_type):
    ...
    Multiple is a boolean parameter. If it is true the template
    is filled multiple times so, we return a dictionary with
    ...
    with codecs.open(path, 'r', encoding='utf-8') as f:
        if(config.readfp(f) == []):
            exit('The settings file is empty or it\'s not where it\'s '
                 'supposed to be.\n It should be under {0} (I was '
                 'looking for this file: {1})'.format(PROPERTIES_PATH, path))
    ...
    if (template_type not in MULTIPLE_PROPERTIES.keys()):
        ...
        for option in options:
            if (template_type not in substitution_words.keys()):
                # The template_type is used as variable for the list
                # in the template
                substitution_words[template_type] = [
                    {MULTIPLE_PROPERTIES[template_type][0]: unicode(option),
                     MULTIPLE_PROPERTIES[template_type][1]: unicode(
                         config.get(section, option))
                     }]
            else:
                substitution_words[template_type].append(
                    {MULTIPLE_PROPERTIES[template_type][0]: unicode(option),
                     MULTIPLE_PROPERTIES[template_type][1]: unicode(
                         config.get(section, option))
                     })
...
def get_filepath_ontology(ontology_id, filetype):
    """ Retun layouts filenames inside a dictionary,
    the key is the level id and the value is the filename

    Keyword:
    ontology_id -- ontology id of the report.
                   Every ontology has its own layout configuration
                   in settings file
    filetype -- file type of the filenames we are looking for
                (layouts or activities).
    ...
    ontology_settings_file = join(ontology_settings_path,
...
# TODO: get_layout_settings and get_children_settings are
# just looking in different sections. Merge!
